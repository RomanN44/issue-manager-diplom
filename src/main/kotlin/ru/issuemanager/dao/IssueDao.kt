...
class IssueDao(
    ...
    fun selectFromIssueByUser(id: Long) = try {
        val sql = "select issue_id, issues.title, issues.description, status, priority, date_start, date_finish, groups.title as groupTitle" +
    ...
    fun selectFromIssueByGroup(id: Long) = try {
        val sql = "select issue_id, issues.member_id, title, description, status, priority, date_start, " +
    ...
    fun insertIssue(request: CreateIssueRequest) = try {
        val sql = "insert into issues(" +
                " member_id, title, description, status, date_start, date_finish, group_id, priority)" +
                " values (null, '${request.title}'," +
                " '${request.description}', 'ToDo', now(), null, ${request.groupId}, ${request.priority});"
    ...
    fun selectIssuesForReport(id: Long) = try {
        val sql = "select title, description, status, priority, date_start, date_finish, users.full_name, users.email" +
                " from issues left join members on issues.member_id = members.member_id " +
                " left join users on members.user_id = users.user_id " +
                " where issues.group_id = $id"
        jdbcTemplate.queryForList(sql)
